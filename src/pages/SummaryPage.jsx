...
export default function FormSummary() {
  ...
  const planPrice = state.isYearly
  ...
  const addonsPrice = state.addons.reduce((result, currentAddon) => {
    return (
      result +
      (state.isYearly ? currentAddon.price.yearly : currentAddon.price.monthly)
    );
  }, 0);
        ...
        <div className="flex justify-between items-center">
          <div>
            <div className="font-medium">
              Arcade ({state.isYearly ? "Yearly" : "Montly"})
            </div>
            <button
              type="button"
          ...
          <span className="font-bold">
            S.P {planPrice}/{termSlug}
          </span>
        </div>
        {hasAddons && (
          <ul className="mt-3 border-t-gray border-t-[1px] text-gray">
            {addons}
          </ul>
        )}
      </div>
      <div className="flex justify-between mt-6 mx-4">
        <span className="text-gray text-body-m">
          Total (per {state.isYearly ? "year" : "month"})
        </span>
        <span className="text-purple font-bold">
          S.P {totalPrice}/{termSlug}
        </span>
      </div>
    </div>
  );
